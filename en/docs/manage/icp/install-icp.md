...
- `CREATE TABLE`, `CREATE VIEW`, `CREATE INDEX`, and `CREATE TRIGGER`
- `CREATE SEQUENCE`, because primary keys use `GENERATED BY DEFAULT ON NULL AS IDENTITY`
- `ADMINISTER DATABASE TRIGGER`, because the script installs an `AFTER LOGON ON SCHEMA` trigger
...
For Oracle, `dbName` is the service name of the database or pluggable database (PDB), not a SID. Set `dbUseTLS` to `true` when the endpoint uses TCPS, as Oracle Autonomous Database does.
